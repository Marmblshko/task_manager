class TasksController < ApplicationController
  ...
  def load_tasks
    @tasks = if current_user.role == 'Admin'
               ...
               tasks_with_current_user = Task.joins(:project).includes(:project).select do |task|
                 task.project.users_in_project.include?(current_user.id)
               end
  ...
  def find_projects
    @projects = Project.where(Project.arel_table[:users_in_project].contains([current_user.id]))
                       .or(Project.where(creator_id: current_user.id)).includes([:users])
  end
  ...
  def task_params
    params.require(:task).permit(:title, :description, :status, :project_id, reports_attributes: %i[title description])
  end
end
